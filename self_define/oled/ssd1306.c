 ...
 * @param Font      字体样式（如Font_7x10）
 * @param color     文字颜色（White或Black）
 * @param x         起始列位置（0-127）
 * @param y         起始行位置（0-7），取决于字体高度
 * 
 * @note 该函数会自动设置光标位置，然后写入字符串
 * @note 行位置(y)的有效范围取决于所选字体的高度
 * @note 超出屏幕边界的内容将被自动截断
 * 
 * 示例：
 * ssd1306_WriteStringAt("Hello World!", Font_7x10, White, 10, 2);
 * // 在第10列、第2行写入"Hello World!"
 * 在使用中发现传统的0.96 oled x必须要+2，不然会卡字，所以我的建议引用时 x>=2
 */
void ssd1306_WriteStringAt(char* str, SSD1306_Font_t Font, SSD1306_COLOR color, uint8_t x, uint8_t y) {
